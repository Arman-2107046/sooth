...
class ProductCatalogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $sortOrder = $request->input('sort', 'alphabetical');
        ...
        $products = $query->paginate(12)
            ...
            ->through(fn ($product) => [
                ...
                'slug' => $product->slug,
                'price' => $product->price,
                'old_price' => $product->old_price, // ✅ include old_price
                'status' => $product->status,
                'is_featured' => $product->is_featured,
                'image' => $product->image_1, // 👈 use consistent key
            ]);
    ...
    public function show(string $slug)
    {
        ...
        $relatedProducts = Product::where('id', '!=', $product->id)
            ...
            ->map(fn ($item) => [
                ...
                'slug' => $item->slug,
                'price' => $item->price,
                'old_price' => $item->old_price, // ✅ include old_price here too
                'image' => $item->image_1,
                'status' => $item->status,
